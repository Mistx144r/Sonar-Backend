...
CREATE TABLE `users` (
  ...
  `userProfileCDN` varchar(255) DEFAULT NULL,
  ...
  `ultimoLogin` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `isBanned` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `email_UNIQUE` (`email`),
  KEY `subscriptionId_idx` (`subscription`),
  CONSTRAINT `subsId` FOREIGN KEY (`subscription`) REFERENCES `subscriptions` (`id`)
